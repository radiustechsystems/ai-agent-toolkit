  ...
  return (
    <div className="mt-6 bg-radius-dark/50 rounded-lg p-4 border border-radius-dark">
      ...
      <div className="prose prose-invert max-w-none">
        {content.split('\n').map((paragraph, index) => (
          <p key={`paragraph-${index}-${paragraph.substring(0, 10)}`} className="mb-3 last:mb-0">
            {paragraph}
          </p>
        ))}
      </div>
      <div className="mt-4 pt-3 border-t border-radius-dark/40 text-xs text-gray-400">
        <p className="italic">
          Note: This is the final content after processing through the entire agent workflow. To see
          intermediate outputs from each agent, expand the agent details in the payment section.
        </p>
      </div>
    </div>
  );
