      ...
      <div className="mb-6 pb-4 border-b border-radius-dark/30">
        ...
        <div className="mb-2">
          ...
          <div className="space-y-3">
            {payments.filter(p => !p.isAgentToAgent).map((payment) => (
              <div 
                    ...
                    agentContributions[payment.agent].servicesRequested!.length > 0 && (
                      <div className="mb-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {agentContributions[payment.agent].servicesRequested?.map(service => (
                            <span key={service} className="text-xs bg-radius-dark/50 px-2 py-1 rounded">
                              {service}
                            </span>
                          ))}
                        </div>
                        ...
                        {payment.role === "creator" ? "Original Content:" : 
                          payment.role === "editor" ? "Edited Content:" : 
                            payment.role === "factChecker" ? "Fact-Checked Content:" : 
                              payment.role === "researcher" ? "Research:" :
                                payment.role === "reviewer" ? "Review:" :
                                  "Contribution:"}
                      </div>
        ...
        {payments.filter(p => p.isAgentToAgent).length > 0 && (
          <div className="mt-6">
            ...
            <div className="space-y-3">
              {payments.filter(p => p.isAgentToAgent).map((payment, idx) => {
                ...
                return (
                  <div 
                    ...
                    style={{ borderColor: payment.service === "research" ? "#FF5722" : 
                      payment.service === "editing" ? "#059669" : 
                        payment.service === "review" ? "#9C27B0" : "#4F46E5" }}
                  >
                    <div className="flex flex-col">
                      ...
                      {payment.tokenCount && (
                        <div className="text-xs text-gray-400 mt-1">
                          {payment.tokenCount} tokens processed at {(payment.amount / payment.tokenCount).toFixed(8)} ETH/token
                        </div>
                      )}
                      
                      <div className="mt-2 flex text-xs text-gray-500">
                        <div className="truncate max-w-32">
                          {payment.agent.substring(0, 6)}...{payment.agent.substring(payment.agent.length - 4)}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-xs font-medium">{payments.filter(p => !p.isAgentToAgent).length}</span>
            </div>
            ...
            {metrics.agentToAgentTransactions && metrics.agentToAgentTransactions > 0 && (
              <div className="flex justify-between">
                ...
                <span className="text-xs font-medium text-radius-primary">{metrics.agentToAgentTransactions}</span>
              </div>
            )}
