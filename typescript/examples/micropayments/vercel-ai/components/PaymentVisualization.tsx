...
export function PaymentVisualization({ payments }: PaymentVisualizationProps) {
  ...
  useEffect(() => {
    ...
    activeIntervalsRef.current.forEach(interval => window.clearInterval(interval));
    ...
    const titleGroup = labelLayer.append("g")
      .attr("class", "title")
    ...
    const hintTooltip = d3.select("body").append("div")
    ...
    agentToAgentPayments.forEach(payment => {
    ...
    if (agents.length === 6) {
      ...
      const creator = agents.find((a: any) => a.role === "creator");
      const editor = agents.find((a: any) => a.role === "editor");
      const researcher = agents.find((a: any) => a.role === "researcher");
      const factChecker = agents.find((a: any) => a.role === "factChecker");
      const reviewer = agents.find((a: any) => a.role === "reviewer");
      const translator = agents.find((a: any) => a.role === "translator");
      ...
      agentNodes = agents.map(agent => {
        ...
        const position = positionMap.get(agent.id) || {
          x: serviceNode.x + radius * Math.cos((agents.indexOf(agent) * Math.PI * 2) / agents.length),
          y: serviceNode.y + radius * Math.sin((agents.indexOf(agent) * Math.PI * 2) / agents.length)
        };
    ...
    agentNodes.filter(agent => agent.receivesBasePayment).forEach((agent, index) => {
    ...
    agentGroups
      ...
      .attr("fill", d => {
        switch (d.role) {
        case "creator": return "rgba(79, 70, 229, 0.3)";  // Blue
        case "editor": return "rgba(5, 150, 105, 0.3)";   // Green
        case "factChecker": return "rgba(219, 39, 119, 0.3)"; // Pink
        case "researcher": return "rgba(255, 87, 34, 0.3)";   // Orange
        case "reviewer": return "rgba(156, 39, 176, 0.3)";    // Purple
        case "translator": return "rgba(0, 188, 212, 0.3)";   // Cyan
    ...
    agentGroups
      ...
      .attr("stroke", (d) => {
        switch (d.role) {
        case "creator": return "#4F46E5";    // Blue
        case "editor": return "#059669";     // Green
        case "factChecker": return "#DB2777"; // Pink
        case "researcher": return "#FF5722";  // Orange
        case "reviewer": return "#9C27B0";    // Purple
        case "translator": return "#00BCD4";  // Cyan
    ...
    agentNodes.forEach((agent, i) => {
      labelLayer
        .append("text")
        .attr("x", agent.x)
        .attr("y", agent.y + agent.radius + 30) // More space from node 
        .attr("text-anchor", "middle")
        .attr("fill", "#FFFFFF")
        .style("font-size", "16px") // Larger text
        .style("font-weight", "bold")
    ...
    agentNodes.filter(agent => agent.receivesBasePayment).forEach((agent, index) => {
      ...
      const tooltipArea = labelLayer.append("circle") 
        .attr("cx", agent.x)
        .attr("cy", agent.y)
      ...
      const tooltip = d3.select("body").append("div")
        ...
        .style("border", () => {
          switch (agent.role) {
          case "creator": return "2px solid #4F46E5";
          case "editor": return "2px solid #059669";
          case "factChecker": return "2px solid #DB2777";
          case "researcher": return "2px solid #FF5722";
          case "reviewer": return "2px solid #9C27B0";
          case "translator": return "2px solid #00BCD4";
          default: return "2px solid #4F46E5";
          }
        })
        .style("border-radius", "8px")
        .style("padding", "10px")
  ...
  agent.role === "creator" ? "✍️" : 
    agent.role === "editor" ? "📝" : 
      ...
      const pulseIndicator = window.setInterval(() => {
        ...
        if (document.querySelector(`.agent-tooltip-area-${index}`)) {
          const currentR = parseFloat(tooltipArea.attr("r"));
    ...
    agentNodes.filter(agent => agent.receivesBasePayment).forEach((agent, index) => {
      ...
      // Create a group for each token so we can add a glow (in label layer for top z-index)
        const tokenGroup = labelLayer.append("g")
        ...
        tokenGroup.append("circle")
          ...
          .attr("fill", function() {
            ...
            case "researcher": return "rgba(255, 87, 34, 0.6)";
            case "reviewer": return "rgba(156, 39, 176, 0.6)";
            case "translator": return "rgba(0, 188, 212, 0.6)";
        ...
        }, 800 + index * 200 + i * 300); // Staggered delay
    ...
    agentToAgentPayments.forEach((payment, index) => {
    // Find the source and target nodes with proper typing
      const sourceNode = agentNodes.find((node: any) => node.id === payment.agent);
      const targetNode = agentNodes.find((node: any) => node.id === payment.to);
      ...
      const controlPoint = {
        x: (sourceNode.x + targetNode.x) / 2 + (Math.random() * 30 - 15),
        y: (sourceNode.y + targetNode.y) / 2 + (Math.random() * 30 - 15)
      };
      
      // Create a dashed path for agent-to-agent payments with glow effect (in connection layer)
      connectionLayer
        .append("path")
        .attr("d", `M${sourceNode.x},${sourceNode.y} Q${controlPoint.x},${controlPoint.y} ${targetNode.x},${targetNode.y}`)
      ...
      if (payment.service) {
        ...
        if (pathNode) {
          ...
          const hoverPath = labelLayer.append("path")
            .attr("d", `M${sourceNode.x},${sourceNode.y} Q${controlPoint.x},${controlPoint.y} ${targetNode.x},${targetNode.y}`)
          ...
          const serviceTooltip = d3.select("body").append("div")
          ...
          const pulseInterval = window.setInterval(() => {
            ...
            if (document.querySelector(`.agent-payment-path-${index}`)) {
              ...
              const currentOpacity = parseFloat(agentPath.attr("opacity") || "1");
    ...
    const legend = legendGroup
      ...
      .attr("transform", (d, i) => `translate(${i * itemSpacing}, 0)`); // Even spacing between items
  ...
  return (
    <div className="mt-4 mb-10 bg-gradient-to-br from-gray-900/90 to-gray-950/90 p-6 rounded-xl border border-indigo-500/30 shadow-lg">
      <h3 className="text-2xl font-bold mb-4 text-white flex items-center">
        ...
        <span className="text-sm ml-3 font-normal text-gray-300 bg-black/30 px-3 py-1 rounded-full">Real-time transaction flow</span>
      </h3>
