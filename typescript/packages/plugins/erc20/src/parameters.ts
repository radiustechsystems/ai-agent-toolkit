...
export class GetTokenBalanceParameters extends createToolParameters(
  z.object({
    ...
    decimals: z.number().optional().describe("The number of decimals of the token (optional, will attempt to auto-detect)"),
  }),
) {}
...
export class TransferParameters extends createToolParameters(
  z.object({
    ...
    // eslint-disable-next-line max-len
    amount: z.string().describe("The amount of tokens to transfer (in base units by default, or decimal units if formatAmount=true)"),
    // eslint-disable-next-line max-len
    formatAmount: z.boolean().optional().default(false).describe("If true, amount is in decimal units and will be converted to base units"),
    // eslint-disable-next-line max-len
    decimals: z.number().optional().describe("The number of decimals to use when parsing amount (optional, will attempt to auto-detect)"),
  }),
) {}
...
export class ApproveParameters extends createToolParameters(
  z.object({
    ...
    // eslint-disable-next-line max-len
    amount: z.string().describe("The amount of tokens to approve (in base units by default, or decimal units if formatAmount=true)"),
    // eslint-disable-next-line max-len
    formatAmount: z.boolean().optional().default(false).describe("If true, amount is in decimal units and will be converted to base units"),
    // eslint-disable-next-line max-len
    decimals: z.number().optional().describe("The number of decimals to use when parsing amount (optional, will attempt to auto-detect)"),
...
export class ConvertToBaseUnitParameters extends createToolParameters(
  z.object({
    // eslint-disable-next-line max-len
    amount: z.union([z.number(), z.string()]).describe("The amount of tokens to convert from decimal units to base units"),
...
export class ConvertFromBaseUnitParameters extends createToolParameters(
  z.object({
    // eslint-disable-next-line max-len
    amount: z.union([z.number(), z.string(), z.bigint()]).describe("The amount of tokens to convert from base units to decimal units"),
    decimals: z.number().describe("The number of decimals of the token"),
  }),
) {}
