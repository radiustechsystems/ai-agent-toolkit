...
export class Erc20Service {
  ...
  async getTokenBalance(
    ...
    try {
      ...
      const token = this.tokens.find((t) =>
        Object.values(t.chains).some(
          (c) => c.contractAddress.toLowerCase() === parameters.tokenAddress.toLowerCase(),
        ),
      );
  ...
  async transfer(walletClient: RadiusWalletInterface, parameters: TransferParameters) {
    try {
      ...
      throw new TransactionError(
        `Failed to transfer token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async revokeApproval(walletClient: RadiusWalletInterface, parameters: RevokeApprovalParameters) {
    try {
      ...
      throw new TransactionError(
        `Failed to revoke token approval: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async transferFrom(walletClient: RadiusWalletInterface, parameters: TransferFromParameters) {
    try {
      ...
      throw new TransactionError(
        `Failed to transfer token from: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async convertToBaseUnit(parameters: ConvertToBaseUnitParameters) {
    try {
      ...
      throw new Error(
        `Failed to convert to base units: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async convertFromBaseUnit(parameters: ConvertFromBaseUnitParameters) {
    try {
      ...
      throw new Error(
        `Failed to convert from base units: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
