...
describe('BatchTransactionHandler', () => {
  ...
  describe('executeSequentialBatch', () => {
    ...
    test('should throw BatchTransactionError when a transaction fails', async () => {
      ...
      await expect(batchHandler.executeSequentialBatch(transactions)).rejects.toThrow(
        /Batch transaction failed at index 1/,
      );
    ...
    test('should throw for invalid transaction format', async () => {
      ...
      await expect(batchHandler.executeSequentialBatch(invalidTx)).rejects.toThrow(
        /Invalid transaction/,
      );
    });
  });
