...
describe('EnsResolver', () => {
  ...
  test('should not resolve ENS names to addresses', async () => {
    await expect(resolver.resolveAddress('test.eth')).rejects.toThrow(/Cannot resolve ENS name/);
  });
  ...
  test('should return lowercase hex address for valid addresses', async () => {
    ...
    const mixedCaseAddress = await resolver.resolveAddress(
      '0x1234567890123456789012345678901234ABCDEF',
    );
  ...
  test('should throw for invalid ENS names', async () => {
    await expect(resolver.resolveAddress('invalid.eth')).rejects.toThrow(AddressResolutionError);
  });

  test('should throw for non-ENS and non-address strings', async () => {
    await expect(resolver.resolveAddress('not-an-address')).rejects.toThrow(AddressResolutionError);
  });
