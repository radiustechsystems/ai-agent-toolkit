...
import type { Account } from '@radiustechsystems/sdk';
  ...
  return {
    Account: vi.fn().mockImplementation(() => ({
      ...
      signMessage: vi
        ...
        .mockResolvedValue(
          new Uint8Array([
            0x6d, 0x6f, 0x63, 0x6b, 0x73, 0x69, 0x67, 0x6e, 0x61, 0x74, 0x75, 0x72, 0x65,
          ]),
        ),
    })),
    Address: vi.fn(),
...
describe('TypedDataSigner', () => {
  ...
  test('should throw SigningError when signing fails', async () => {
    ...
    await expect(signer.signTypedData(mockFailingAccount, typedData)).rejects.toThrow(SigningError);
  });
