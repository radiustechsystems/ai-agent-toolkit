    ...
    for (const toolProvider of this.toolProviders) {
      ...
      if (!toolsMap) {
        ...
        if (constructorName === 'Function') {
          console.warn(
            `
              Detected a non-instance tool provider. 
              Please ensure you're passing instances of your tool providers, 
              by using \`new MyToolProvider(..)\`
                        `
              .trim()
              .replace(/\n\s+/g, ' '),
          );
          ...
          console.warn(
            `
              No tools found for ${constructorName}. 
              Please ensure you're using the '@Tool' decorator to expose your tools.
            `
              .trim()
              .replace(/\n\s+/g, ' '),
          );
